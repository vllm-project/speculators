...
from model.modeling_llama_TTT_eagle3 import LlamaDecoderLayer, LlamaRotaryEmbedding, LlamaRMSNorm
try:
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=False, total_tokens=63,  depth=5, top_k=8, threshold=1.0):
        ...
        self.embed_tokens = nn.Embedding(config.target_vocab_size, config.hidden_size, self.padding_idx)
    ...
    def forward(self,
        ...
        for i in range(len(hidden_states_history)):
            ...
            prior_hidden_states[i]=(torch.cat((inputs_embeds, prior_hidden_states[i]), dim=-1))
