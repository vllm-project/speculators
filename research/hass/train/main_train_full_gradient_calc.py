...
def compute_loss(target, target_p, predict, loss_mask, head):
        ...
        for batch_idx, data in enumerate(tqdm(train_loader)):
            with accelerator.accumulate(model):
                ...
                for forward_idx in range(args.forward_num_total):
                    ...
                    ploss, topk_loss, out_head , kldiv_loss = compute_loss(target, target_p, pred, loss_mask, head)
                    total_loss = train_config["p_w"] * ploss  + train_config['kldiv_w']*kldiv_loss
                    loss += total_loss
                    accelerator.backward(total_loss)
                    hidden_states_history.append(hidden_states.detach())
                    hidden_states=torch.concat([hidden_states[:, :1, :].detach(), predict[:, :-1, :].detach()], dim=1)
