...
import torch
from transformers import Mistral3ForConditionalGeneration, AutoTokenizer, AutoProcessor
# from transfomrers import 
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples['id'])):
            ...
            for response in responses:
                ...
                n=len(search)
                matches=[i for i in range (len(input_ids.tolist())-n+1) if input_ids.tolist()[i:i+n]==search.tolist()]
...
# print(ds)
bigmodel = Mistral3ForConditionalGeneration.from_pretrained(bigname,  device_map="auto",torch_dtype=torch.float16)
bigmodel.eval()
