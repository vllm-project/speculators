...
declare -A GPU_TO_FIXED_PREFILL_LENGTH=(
  [0]=1
  [1]=16
  [2]=32
  [3]=64
...
for GPU in "${!GPU_TO_FIXED_PREFILL_LENGTH[@]}"; do
  FIXED_PREFILL_LENGTH="${GPU_TO_FIXED_PREFILL_LENGTH[$GPU]}"
  run_series_on_gpu "$GPU" "$FIXED_PREFILL_LENGTH" &
done
