...
: "${FIXED_PREFILL_LENGTH:=-1}"
: "${FIXED_DECODE_LENGTH:=-1}"
...
# — model & spec settings —
# MODEL_PATH="/home/linghao/models/Qwen3-8B/"
...
# SPEC_MODEL_PATH="/nm/drive0/linghao/Qwen3-8B-speculator-ckpt/state_-1"
...
PROMPT_TOKENS=$FIXED_PREFILL_LENGTH
OUTPUT_TOKENS=$FIXED_DECODE_LENGTH
...
# OUTPUT_PATH=~/speculators/throughput/output_"${MODEL_NAME}"_"${SPEC_SETTING}"_draft-"${NUM_SPEC_TOKENS}"_prompt-"${PROMPT_TOKENS}"_output-"${OUTPUT_TOKENS}".json
OUTPUT_PATH=~/speculators/throughput/output_"${MODEL_NAME}"_"${SPEC_SETTING}"_prompt-"${PROMPT_TOKENS}"_output-"${OUTPUT_TOKENS}_no_max_concurrency".json
# — activate conda env —
...
FIXED_PREFILL_LENGTH="$FIXED_PREFILL_LENGTH" \
FIXED_DECODE_LENGTH="$FIXED_DECODE_LENGTH" \
