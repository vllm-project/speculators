...
3. **Push and create draft PR** on the speculators repo:
   ```bash
   cd /workspace/speculators
   git push -u myfork feat/<algo_name>
   gh pr create --draft --repo vllm-project/speculators \
...
4. **If vLLM changes exist**, push and create a draft PR there too:
   ```bash
   cd /workspace/vllm
   git push -u myfork feat/<algo_name>
   gh pr create --draft --repo vllm-project/vllm \
