...
3. **Push and create draft PR** on the speculators repo. **Capture the PR URL** from `gh pr create` output:
   ...
   SPEC_PR_URL=$(gh pr create --draft --repo vllm-project/speculators \
     ...
     --body "$(cat <<'PREOF'
   > **Note:** This PR was auto-generated by the [oneshot paper-to-implementation agent](https://github.com/vllm-project/speculators/tree/main/.claude/skills/oneshot-paper). It requires human review before merging.
