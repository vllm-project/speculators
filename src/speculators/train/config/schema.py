...
class DraftArgs(_Group):
    ...
    num_layers: int | None = Field(
        ...
        "(default: 5 for dflash and dflash2, 1 otherwise).",
    )
...
class TrainConfig(BaseSettings):
    ...
    def _resolve_derived_defaults(self) -> "TrainConfig":
        ...
        for dflash/dflash2 else ``1``; unset ``per_position_loss_weight`` -> ``dpace``
        for dflash else ``fixed-exp-decay``; unset ``loss_fn`` -> ``ce`` for dflash
        else ``kl_div``; unset ``block_size`` -> ``16`` for dflash else ``8``.
        ...
        uses_five_layers = self.speculator_type in {"dflash", "dflash2"}
        ...
        if self.draft.num_layers is None:
            self.draft.num_layers = 5 if uses_five_layers else 1
