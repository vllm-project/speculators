...
def create_transformer_layer_config(  # noqa: C901
    ...
    heterogeneous verifiers they are resolved from a representative per-layer
    config rather than the (ambiguous) global config.
    """
    ...
    if getattr(verifier_config, "is_heterogeneous", False):
        ...
        def _geometry(layer_cfg: PretrainedConfig) -> tuple[Any, ...]:
            """Per-layer attribute values, as a hashable key for grouping layers."""
    ...
    def _verifier_attr(name: str, default: Any = _UNSET) -> Any:
        """Read a verifier attribute, from the representative layer if per-layer."""
