...
def _encode_render(
    ...
    *,
    ...
    """Render a conversation prefix with the chat template and return token ids."""
    hf_conv = _adapt_conv_for_hf(conv_prefix, processor)
    ...
    if isinstance(processor, ProcessorMixin):
        ...
        rows = encoded["input_ids"]  # type: ignore[index]
        if hasattr(rows, "tolist"):
            rows = rows.tolist()
        assert not isinstance(rows, str)
        (input_ids,) = rows  # remove batch dimension
        return input_ids
    ...
    # Special tokens (e.g. BOS) come from the template itself
    return processor(text, add_special_tokens=False)["input_ids"]
...
def _passthrough_pretokenized(
    ...
    need no chat-template rendering.
    """
...
def _preprocess_batch(
    ...
    num_convs_in = 0
    num_convs_empty = 0
    num_rows_out_before = len(results["input_ids"])
    ...
    for idx, conv in enumerate(conversations):
        ...
        try:
            ...
            log.error(f"Failed to process conversation {idx}: {e}")
            num_convs_empty += 1
            continue
    ...
    num_rows = len(results["input_ids"]) - num_rows_out_before
...
def build_eagle3_dataset(
    ...
    *,
    ...
    The loss mask is the render boundary of each assistant turn in the
    processor's chat template (see ``_render_boundary_rows``); append-only
    templates keep one row per conversation, history-rewriting templates
    (e.g. reasoning models) fan out to one row per assistant turn.
    ...
    # passes them through: no chat template, no boundary rendering, no turn dropout.
    ...
    if pretokenized:
        ...
        log.info("Deriving loss masks from chat-template render boundaries")
