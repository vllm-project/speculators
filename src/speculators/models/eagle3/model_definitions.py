...
def all_to_all_4d(
    ...
    scatter_dim = 2
    if scatter_idx == scatter_dim and gather_idx == 1:
        # input (torch.tensor): a tensor sharded along dim 1
        # (bs, seqlen/P, hc, hs) output: (bs, seqlen, hc/P, hs)
        ...
        input_t = (
            x.reshape(bs, shard_seqlen, seq_world_size, shard_hc, hs)
            .transpose(0, 2)
            .contiguous()
        )
        ...
        # input (torch.tensor): a tensor sharded along dim 1
        # (bs, seqlen, hc/P, hs) output: (bs, seqlen/P, hc, hs)
...
class LlamaUSPFlashAttention(nn.Module):
    ...
    def ring_attention_hybrid_masked(
        ...
        # bs=1
        _, hidden_size, local_seq_len, head_dim = local_q.shape
        ...
        for step in range(world_size):
            ...
            l_new = loss * alpha + p_block.sum(dim=-1)
        ...
        # 3. Finalize
        # =============================================================
        final_output = acc / (loss.unsqueeze(-1) + 1e-6)
