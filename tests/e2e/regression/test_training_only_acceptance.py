...
def _build_vocab_mappings(
    ...
    token_freq_dict = torch.load(
        token_freq_dir / "token_freq.pt", weights_only=True
    )
...
def _resolve_vocab_mappings(config: dict, tmp_path: Path) -> tuple[Path, Path]:
    ...
    if "vocab_mapping_repo" in config:
        mapping_dir = Path(
            snapshot_download(
                repo_id=config["vocab_mapping_repo"], repo_type="dataset"
            )
        )
        ...
        token_freq_dir = Path(
            snapshot_download(
                repo_id=config["token_freq_repo"], repo_type="dataset"
            )
        )
...
def test_training_acceptance(config: dict, tmp_path: Path):
    ...
    hidden_states_dir = snapshot_download(
        repo_id=config["hidden_states_repo"], repo_type="dataset"
    )
