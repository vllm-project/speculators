...
from speculators.utils.transformer_utils import (
...
class TestEagleConverter:
    ...
    def test_layernorm_weight_mapping(self):
        ...
        assert (
            converter.EAGLE_TO_SPECULATORS_LAYERNORM_MAPPINGS[
    ...
    def test_weight_skipping_and_remapping(self):
        ...
        assert (
            converter._should_skip_weight("embed_tokens.weight", has_layernorms=False)
            is True
        )
        assert (
            converter._should_skip_weight("embed_tokens.weight", has_layernorms=True)
            is True
        )

        # Test hidden_layernorm skipping when layernorms disabled
        assert (
            converter._should_skip_weight(
                "hidden_layernorm.weight", has_layernorms=False
            )
            is True
        )
        assert (
            converter._should_skip_weight(
                "hidden_layernorm.weight", has_layernorms=True
            )
            is False
        )
        ...
        assert (
            converter._remap_weight_name("fc.weight", has_layernorms=False)
            == "fusion_fc.weight"
        )
        assert (
            converter._remap_weight_name("fc.bias", has_layernorms=False)
            == "fusion_fc.bias"
        )

        # Test transformer layer remapping
        assert (
            converter._remap_weight_name(
                "layers.0.self_attn.q_proj.weight", has_layernorms=False
            )
        ...
        assert (
            converter._remap_weight_name("hidden_layernorm.weight", has_layernorms=True)
            == "transformer.input_layernorm.weight"
        )

        # Test layernorm mappings
        assert (
            converter._remap_weight_name("embed_layernorm.weight", has_layernorms=True)
            == "embedding_layernorm.weight"
        )
        assert (
            converter._remap_weight_name(
                "lm_head_layernorm.weight", has_layernorms=True
            )
            == "pre_lm_head_layernorm.weight"
        )
        ...
        assert (
            converter._remap_weight_name("lm_head.weight", has_layernorms=False)
            == "lm_head.weight"
        )
    ...
    def test_process_checkpoint_weights(self):
        ...
        processed = converter._process_checkpoint_weights(
            weights_with_bias, has_layernorms=False
        )
        ...
        processed = converter._process_checkpoint_weights(
            weights_with_layernorms, has_layernorms=True
        )
    ...
    def test_ensure_checkpoint_is_local(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "speculators.convert.eagle.utils.download_checkpoint_from_hub"
            ) as mock_download:
                ...
                mock_download.assert_called_once_with(
                    model_id="non/existent", cache_dir=None
                )
