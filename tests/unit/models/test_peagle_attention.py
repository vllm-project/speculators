...
def _lengths_to_document_ids(lengths, total_seq_len):
    return _build_document_ids(lengths, total_seq_len)
...
class TestNoStreamingUnchanged:
    ...
    def test_single_doc_no_streaming(self):
        ...
        mask_with = create_peagle_mask_mod(
            anchor_pos, depth, lengths, seq_len, sink_size=None, max_context_window=None
        )
        mask_without = create_peagle_mask_mod(anchor_pos, depth, lengths, seq_len)
...
class TestStreamingSingleDoc:
    ...
    def test_sink_and_window(self):
        ...
        # Query at position 15 (depth 0, index 15) attending to depth-0 KVs:
        q_idx = 15
        for kv_idx in range(seq_len):
            kv_pos = kv_idx
            is_sink = kv_pos < sink_size  # 0,1,2,3
            in_window = kv_pos >= (q_idx - window)  # 9..15
    ...
    def test_gap_positions_blocked(self):
        ...
        q_idx = 15
        # Gap positions: 3..10 (not sinks, not in window [11..15])
    ...
    def test_early_query_no_gap(self):
        """When query is near start, window and sink overlap — all positions visible."""
        seq_len = 20
        sink_size = 5
        window = 8
        lengths = torch.tensor([seq_len])
        ...
        # Query at position 6: sinks cover 0-4, window covers max(0,6-8)=0..6
        # So all positions 0..6 should be visible
        q_idx = 6
...
class TestStreamingMultiDoc:
    ...
    def test_cross_doc_sinks_blocked(self):
        ...
        lengths = torch.tensor([doc0_len, doc1_len])
        ...
        # Query in doc1 at position 18
        q_idx = 18
        # Doc0 positions (0-9) should all be blocked
...
class TestWindowCoversAll:
    ...
    def test_large_window_matches_no_streaming(self):
        ...
        mask_full = create_peagle_mask_mod(anchor_pos, depth, lengths, seq_len)
...
class TestRolloutUnaffected:
    ...
    def test_rollout_attention_preserved(self):
        ...
        # For rollout queries (depth > 0), check that their attention to
        # same-rollout KVs is identical
...
class TestPaddingExcluded:
    ...
    def test_padding_blocked(self):
        doc_len = 6
        total_seq_len = 10  # 4 padding positions
        lengths = torch.tensor([doc_len])
        ...
        # All entries should be within doc boundaries (no padding access)
        # Since anchor_pos only covers positions 0..5, and all are in doc0,
        # verify that no query can see positions >= doc_len (padding)
        # (This is implicitly handled since anchor_pos doesn't include padding,
        # but we verify the mask_mod logic itself)
        zero = torch.zeros((), dtype=torch.long)
        for q in range(doc_len):
            # Query a position that would be in padding range if it existed
            result = mask_mod(zero, zero, torch.tensor(q), torch.tensor(q))
            assert result  # self-attention should work for valid positions
