"""Integration tests for PEagleDraftModel forward passes with real weights."""
...
@requires_cuda
class TestPEagleTraining:
    """Forward pass in training mode (PEagle always requires verifier states)."""
    ...
    def test_forward_backward_produces_valid_outputs(
        ...
        draft_tokens, loss, metrics = peagle_model(**batch)
...
class TestPEagleMultiBatch:
    ...
    def test_multi_then_single(self, peagle_model):
    ...
    def test_alternating_batch_sizes(self, peagle_model):
    ...
    def test_varying_loss_masks_across_batches(self, peagle_model):
...
@requires_cuda
class TestPEagleVocabBoundary:
    """Tests with draft vocab mapping and boundary token IDs."""

    @pytest.fixture
    def peagle_draft_vocab_model(self):
        model = make_peagle_model(draft_vocab_size=32)
    ...
    def test_boundary_tokens(self, peagle_draft_vocab_model):
        samples = _make_samples([128], vocab_size=32, boundary_token_ids=[0, 31])
        batch = make_batch(max_len=MAX_LEN, samples=samples, hidden_size=HIDDEN_SIZE)
        draft_tokens, loss, metrics = peagle_draft_vocab_model(**batch)
