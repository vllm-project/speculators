...
class TestLlamaEagleIntegration:
    ...
    def test_save_and_load_config(self, eagle_v1_config):
        """Test saving and loading configuration."""
        with tempfile.TemporaryDirectory() as tmpdir:
            save_path = Path(tmpdir)
            
            # Save config
            eagle_v1_config.save_pretrained(save_path)
            ...
            assert loaded_config.eagle_variant == eagle_v1_config.eagle_variant
            assert loaded_config.fc_bias == eagle_v1_config.fc_bias
            assert loaded_config.use_extra_layernorms == eagle_v1_config.use_extra_layernorms
            assert loaded_config.hidden_size == eagle_v1_config.hidden_size
            assert loaded_config.vocab_size == eagle_v1_config.vocab_size
    
    def test_save_and_load_model(self, eagle_v1_config):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert loaded_model.config.eagle_variant == "eagle_v1"
    ...
    def test_model_with_different_hidden_states_inputs(self, eagle_v1_config):
        """Test model with different hidden states input configurations."""
        # Modify config to use different hidden states
        eagle_v1_config.inputs = ["input_ids", "hidden_states[-2]"]
        model = LlamaEagleSpeculator(eagle_v1_config)
        model.eval()
