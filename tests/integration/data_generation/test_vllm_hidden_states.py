...
@pytest.mark.parametrize("model_path,tensor_parallel_size", [
...
def test_vllm_vs_huggingface_accuracy(model_path, tensor_parallel_size):
    """Test that vLLM hidden states match HuggingFace baseline within acceptable tolerance."""

    test_prompts = [
        "The future of artificial intelligence is bright and full of possibilities that will transform humanity.",
        "In a world where technology advances rapidly, we must carefully consider the ethical implications.",
    ]
    ...
    inputs = tokenizer(test_prompts, return_tensors="pt", padding=True, truncation=True, max_length=2048).to(hf_model.device)
    logger.info(f"Input shape: {inputs['input_ids'].shape}")

    with torch.no_grad():
        hf_output = hf_model(**inputs, output_hidden_states=True)

    # EAGLE3 pattern: hidden_states[3], hidden_states[num_layers // 2 + 1], hidden_states[-3]
    hf_layers = [
    ...
    logger.info(f"GPU memory freed, available: {torch.cuda.mem_get_info()[0] / 1024**3:.2f} GiB")
    ...
    try:
        ...
        # IMPORTANT: Use the SAME tokenizer that was used for HuggingFace to ensure identical tokenization
        ...
        vllm_inputs = vllm_tokenizer(test_prompts, return_tensors="pt", padding=True, truncation=True, max_length=2048)
    ...
    # Check layer IDs
    assert actual_layer_ids == expected_layer_ids, f"Layer IDs mismatch! Got {actual_layer_ids}, expected {expected_layer_ids}"
    logger.info(f"✓ Layer IDs match: {expected_layer_ids}")

    # Check shapes
    assert hf_concat.shape == vllm_concat.shape, f"Shape mismatch! HF: {hf_concat.shape}, vLLM: {vllm_concat.shape}"
    ...
    for i, result in enumerate(vllm_results):
        ...
        assert result['hidden_state'].shape[0] == result['input_ids'].shape[0], "Sequence length mismatch"
        assert result['hidden_state'].shape[1] == hf_concat.shape[2], "Hidden dimension mismatch"
    ...
    assert mean_diff < 0.01, f"Mean difference {mean_diff} too large. Expected layer_ids={expected_layer_ids}"
