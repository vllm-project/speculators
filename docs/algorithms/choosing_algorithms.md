...
**How it works:** The `scripts/response_regeneration/` tooling connects to a running inference server and rewrites dataset responses. The `run_all.sh` orchestration script handles model serving and regeneration together [[source: scripts/response_regeneration/README.md]](https://github.com/vllm-project/speculators/blob/v0.3.0/scripts/response_regeneration/README.md):

```bash
./run_all.sh --model "meta-llama/Llama-3.3-70B-Instruct" --dataset magpie
```

Individual script options support dataset selection, row limits, resumption from a previous run, and concurrency tuning [[source: scripts/response_regeneration/README.md]](https://github.com/vllm-project/speculators/blob/v0.3.0/scripts/response_regeneration/README.md).
