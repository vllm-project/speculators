...
Speculator training data must contain responses produced by the target model. This walkthrough uses [`windchimeran/speculator-tutorial`](https://huggingface.co/datasets/windchimeran/speculator-tutorial), 5K UltraChat conversations whose assistant turns were regenerated by `Qwen/Qwen3-8B` itself, so it is already on-policy for this tutorial's target model. It is in speculator format (`input_ids` and `loss_mask`), so `prepare_data.py` reads it directly:

```bash
# in speculators venv
python scripts/prepare_data.py \
  --model Qwen/Qwen3-8B \
  --data hf:windchimeran/speculator-tutorial:tutorial_regen \
  --output ./output \
  --max-samples 5000 \
  --seq-length 8192
```

To train against a different target model, produce your own on-policy data with [Response Regeneration](response_regeneration.md) and pass the resulting file instead:
