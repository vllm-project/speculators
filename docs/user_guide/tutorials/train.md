...
Speculator training data must contain responses produced by the target model. This walkthrough uses the `tutorial_regen` split of [`inference-optimization/speculators-ci-datasets`](https://huggingface.co/datasets/inference-optimization/speculators-ci-datasets), 5K UltraChat conversations whose assistant turns were regenerated by `Qwen/Qwen3-8B` itself, so it is already on-policy for this tutorial's target model. It is in speculator format (`input_ids` and `loss_mask`), so `speculators prepare-data` reads it directly:
