...
Only relevant for offline mode. Cost scales with the *actual* token count of each sample, not with `--total-seq-len`:
...
# For 10K samples: ~960 GB
# For 50K samples: ~4.8 TB
```

The 5K figure is what this tutorial's dataset actually produced: **463 GB** for 4993 samples averaging 3025 tokens. Using `--total-seq-len` (8192) in place of the average would overestimate it by about 2.7x.
