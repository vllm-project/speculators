#!/usr/bin/env bash
# Shared functions for DeepSeek-V4-Flash acceptance rate evaluation.
...
reserve_gpus() {
    ...
    GPU_IDS=$(chg reserve --gpus "${needed}" --duration "${duration}" --short --note "deepseekv4 acceptance eval" 2>&1)
    local rc=$?
    if [[ $rc -ne 0 ]]; then
